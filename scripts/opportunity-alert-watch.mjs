...
const parseArgs = (argv) => {
  const args = {
    ...
    networks: (process.env.ALERT_NETWORKS || 'ethereum,arbitrum,base').split(',').map((n) => n.trim()).filter(Boolean),
    loanAmountUsd: parseNumber(process.env.ALERT_LOAN_AMOUNT_USD, 12000),
    minNetProfitUsd: parseNumber(process.env.ALERT_MIN_NET_PROFIT_USD, 10),
    ...
    enableDexScreener: parseBoolean(process.env.ALERT_ENABLE_DEXSCREENER, true),
    enableGecko: parseBoolean(process.env.ALERT_ENABLE_GECKO, true),
