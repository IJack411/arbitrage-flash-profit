...
$logFile = Join-Path $logDir "scanner-readiness-healthcheck.log"

$timestamp = Get-Date -Format "yyyy-MM-ddTHH:mm:ssK"
"[$timestamp] starting scanner:readiness:healthcheck" | Out-File -FilePath $logFile -Append -Encoding utf8

Push-Location $RepoRoot
try {
  & npm run scanner:readiness:healthcheck *>> $logFile
